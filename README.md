...
- 각각의 유저는 권한그룹을 통해 접근할 수 있는 팀 저장소가 결정된다. 
- 권한이 있는 팀 저장소내에서 문서내에 각 문단에 수정사항을 요청할 수 있다.
- 각 유저는 팀 저장소내에 있는 다른 팀원들의 수정사항들을 열람하고 수정사항들에 댓글을 달 수 있다.
- 각 유저는 수정사항에 동의할지 말지에 대한 투표를 할 수 있다. 
- 저장소의 팀장은 수정사항에 대한 투표결과와 수정 댓글들을 바탕으로 최종 수정을 승락할 수 있다.
- 각 유저는 타 저장소의 문서 목록은 자유롭게 열람할 수 있으나 문서 내용을 보고싶을 떄는 권한을 요청할 수 있다.
- 요청 받은 권한에 대한 승락은 각 팀 저장소를 관리하는 팀장이 권한의 유효기간을 최대 30일까지 설정하여 승락한다.

## 📌 지식 관리
- 지식을 관리를 위한 문서 작성은 마크 다운 형식을 기반으로한 자체 에디터를 통해 작성된다.
- 팀 저장소는 여러 개의 문서를 가질 수 있으며 각 문서별로 타 부서 유저에 대한 접근 권한을 설정할 수 있다. 
